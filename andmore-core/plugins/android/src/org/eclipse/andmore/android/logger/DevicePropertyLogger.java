...
public class DevicePropertyLogger implements ILogFile {
	...
	public DevicePropertyLogger() {
		...
		Collection<DeviceProfile> allDevices = devices.getAllDevices();
		properties = new HashMap<String, Map<String, String>>(allDevices.size());
		...
		for (DeviceProfile deviceProfile : allDevices) {
			...
			String deviceName = deviceProfile.getName();
			try {
				Collection<String> lines = 
					deviceMonitor.execRemoteApp(devices.getSerialNumberByName(deviceProfile.getName()), "getprop", new NullProgressMonitor());
