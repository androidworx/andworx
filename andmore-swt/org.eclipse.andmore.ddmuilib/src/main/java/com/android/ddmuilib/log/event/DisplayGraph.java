...
public class DisplayGraph extends EventDisplay {
    ...
    void newEvent(EventContainer event, EventLogParser logParser) {
        ...
        ArrayList<DisplayDescriptor> occurrenceDescriptors =
                new ArrayList<DisplayDescriptor>();
    ...
    private void updateChart(EventContainer event, EventLogParser logParser,
        ...
        if (event.mTag == 2721) {
            ...
            for (DisplayDescriptor descriptor : valueDescriptors) {
            	ValueDisplayDescriptor valueDisplayDescriptor = (ValueDisplayDescriptor)descriptor;
        ...
        for (DisplayDescriptor descriptor : valueDescriptors) {
            try {
                ...
                if (map == null) {
                    ...
                    mValueDescriptorSeriesMap.put(valueDisplayDescriptor, map);
                }
                ...
                if (timeSeries == null) {
                    ...
                    switch (mValueDescriptorCheck) {
                        ...
                        default:
                            seriesFullName = String.format("%1$s / %2$s: %3$s", seriesLabel,
                                    tagMap.get(descriptor.getEventTag()),
                                    valueDisplayDescriptor.valueName);
                            break;
                    }
                    ...
                    TimeSeriesCollection dataset = getValueDataset(
                            logParser.getEventInfoMap().get(event.mTag)[valueDisplayDescriptor.valueIndex]
                                                                        .getValueType(),
                            accumulateValues);

                    // create the series
                    timeSeries = new TimeSeries(seriesFullName);
        ...
        for (DisplayDescriptor descriptor : occurrenceDescriptors) {
            try {
                // get the hashmap for this descriptor
                HashMap<Integer, TimeSeries> map = mOcurrenceDescriptorSeriesMap.get(descriptor);
                OccurrenceDisplayDescriptor occurrenceDisplayDescriptor = (OccurrenceDisplayDescriptor)descriptor;
                // if it's not there yet, we create it.
                if (map == null) {
                    map = new HashMap<Integer, TimeSeries>();
                    mOcurrenceDescriptorSeriesMap.put(occurrenceDisplayDescriptor, map);
                }
                ...
                // if it doesn't exist yet, we create it.
                if (timeSeries == null) {
                    String seriesLabel = getSeriesLabel(event, occurrenceDisplayDescriptor);

                    String seriesFullName = String.format("[%1$s:%2$s]",
                            tagMap.get(descriptor.getEventTag()), seriesLabel);
    ...
    private String getChartTitle(EventLogParser logParser) {
        if (mValueDescriptors.size() > 0) {
            ...
            switch (mValueDescriptorCheck) {
                case EVENT_CHECK_SAME_TAG:
                    if (logParser != null) {
                        chartDesc = logParser.getTagMap().get(mValueDescriptors.get(0).getEventTag());
                    }
                    break;
                case EVENT_CHECK_SAME_VALUE:
                    if (logParser != null) {
                        chartDesc = String.format("%1$s / %2$s",
                                logParser.getTagMap().get(mValueDescriptors.get(0).getEventTag()),
                                ((ValueDisplayDescriptor)mValueDescriptors.get(0)).valueName);
                    }
                    break;
            }
