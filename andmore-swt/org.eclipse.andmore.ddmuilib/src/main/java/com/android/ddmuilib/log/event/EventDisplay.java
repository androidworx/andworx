...
abstract class EventDisplay {
     ...
     */
    static class OccurrenceDisplayDescriptor implements DisplayDescriptor {
        ...
        /* (non-Javadoc)
		 * @see com.android.ddmuilib.log.event.DiplayDescriptor#replaceWith(com.android.ddmuilib.log.event.EventDisplay.OccurrenceDisplayDescriptor)
		 */
        @Override
		public void replaceWith(OccurrenceDisplayDescriptor descriptor) {
        ...
        /* (non-Javadoc)
		 * @see com.android.ddmuilib.log.event.DiplayDescriptor#loadFrom(java.lang.String[], int)
		 */
        @Override
		public int loadFrom(String[] storageStrings, int index) {
        ...
        /* (non-Javadoc)
		 * @see com.android.ddmuilib.log.event.DiplayDescriptor#getStorageString()
		 */
        @Override
		...
		@Override
		public CompareMethod getFilterCompareMethod() {
			return filterCompareMethod;
		}
    ...
    protected final ArrayList<DisplayDescriptor> mValueDescriptors = new ArrayList<>();
    private final ArrayList<DisplayDescriptor> mOccurrenceDescriptors = new ArrayList<>();
    ...
    static EventDisplay clone(EventDisplay from) {
        ...
        for (DisplayDescriptor desc : from.mValueDescriptors) {
            ed.mValueDescriptors.add(new ValueDisplayDescriptor((ValueDisplayDescriptor)desc));
        }
        ed.mValueDescriptorCheck = from.mValueDescriptorCheck;

        for (DisplayDescriptor desc : from.mOccurrenceDescriptors) {
            ed.mOccurrenceDescriptors.add(new OccurrenceDisplayDescriptor((OccurrenceDisplayDescriptor)desc));
        }
        return ed;
    }
    ...
    Iterator<ValueDisplayDescriptor> getValueDescriptors() {
    	return new Iterator<ValueDisplayDescriptor> () {
            private Iterator<DisplayDescriptor> iterator = mValueDescriptors.iterator();
			@Override
			...
			@Override
			public ValueDisplayDescriptor next() {
				return (ValueDisplayDescriptor)iterator.next();
			}};
    }
    ...
    Iterator<OccurrenceDisplayDescriptor> getOccurrenceDescriptors() {
    	return new Iterator<OccurrenceDisplayDescriptor> () {
            private Iterator<DisplayDescriptor> iterator = mOccurrenceDescriptors.iterator();
			@Override
			public boolean hasNext() {
				return iterator.hasNext();
			}
			...
			@Override
			public OccurrenceDisplayDescriptor next() {
				return (OccurrenceDisplayDescriptor)iterator.next();
			}};
    }
     ...
     */
    DisplayDescriptor getDescriptor(
            Class<? extends DisplayDescriptor> descriptorClass, int index) {

    if (descriptorClass.isInstance(ValueDisplayDescriptor.class)) {
        return mValueDescriptors.get(index);
    } else if (descriptorClass.isInstance(OccurrenceDisplayDescriptor.class)) {
     ...
     */
    void removeDescriptor(Class<? extends DisplayDescriptor> descriptorClass, int index) {
        if (descriptorClass.isInstance(OccurrenceDisplayDescriptor.class)) {
            mOccurrenceDescriptors.remove(index);
        } else if (descriptorClass.isInstance(ValueDisplayDescriptor.class)) {
    ...
    private String getDescriptorStorageString(
            ArrayList<DisplayDescriptor> descriptorList) {
        StringBuilder sb = new StringBuilder();
        boolean first = true;

        for (DisplayDescriptor descriptor : descriptorList) {
            if (first == false) {
    ...
    private void getDescriptors(EventContainer event,
            ArrayList<DisplayDescriptor> fullList,
            ArrayList<DisplayDescriptor> outList) {
        for (DisplayDescriptor descriptor : fullList) {
            try {
                // first check the event tag.
                if (descriptor.getEventTag() == event.mTag) {
                    // now check if we have a filter on a value
                    if (descriptor.getFilterValueIndex() == -1 ||
                            event.testValue(descriptor.getFilterValueIndex(), descriptor.getFilterValue(),
                                    descriptor.getFilterCompareMethod())) {
                ...
                Log.e("Event Log", String.format(
                        ...
                        descriptor.getFilterValueIndex(), descriptor.getEventTag()));
            }
        }
    }
    ...
    protected boolean filterEvent(EventContainer event,
            ArrayList<DisplayDescriptor> valueDescriptors,
            ArrayList<DisplayDescriptor> occurrenceDescriptors) {
    ...
    private int checkDescriptors() {
        ...
        int tag = -1;
        int index = -1;
        for (DisplayDescriptor display : mValueDescriptors) {
            if (tag == -1) {
                tag = display.getEventTag();
                index = ((ValueDisplayDescriptor)display).valueIndex;
            } else {
                if (tag != display.getEventTag()) {
                    return EVENT_CHECK_FAILED;
                } else {
                    if (index != -1) {
                        if (index != ((ValueDisplayDescriptor)display).valueIndex) {
                            index = -1;
                        }
                    }
                }
