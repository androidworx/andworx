...
public class DeviceChooserDialog extends Dialog implements IDeviceChangeListener {
    ...
    private final IEditorIconFactory mIconFactory;
    ...
    private class ContentProvider implements IStructuredContentProvider {
        ...
        private Object[] findCompatibleDevices(IDevice[] devices) {
            ...
            for (IDevice device : devices) {
                AndroidVersion deviceVersion = Utilities.getDeviceVersion(device);
                ...
                switch (columnIndex) {
                    ...
                    case 2:
                        ...
                        if (device.isEmulator() == false) { // physical device
                            ...
                            // get the AvdInfo
                            AvdInfo info = mSdkContext.getAvdManager().getAvd(device.getAvdName(),
                                    true /*validAvdOnly*/);
                            if (info == null)
                                return mWarningImage;
                            IAndroidTarget avdTarget = getAndroidTargetFor(info);
                            Utilities.Compatibility c =
                                    Utilities.canRun(info, mProjectTarget, avdTarget, mMinApiVersion);
                            switch (c) {
                                case YES:
                                    return mMatchImage;
                                case NO:
                ...
                switch (columnIndex) {
                    ...
                    case 2:
                        if (device.isEmulator()) {
                            ...
                            return getAndroidTargetFor(info).getFullName();
                        } else {
    ...
    public DeviceChooserDialog(Shell parent, SdkCallAgent sdkCallAgent, DeviceChooserResponse response, String packageName,
    	...
    	mSdkTargets = new SdkTargets(mSdkContext);
    	mIconFactory = sdkCallAgent.getEditorIconFactory();
        mResponse = response;
    ...
    private void loadImages(Display display) {
        ImageFactory imageFactory = mSdkContext.getSdkHelper().getImageFactory();

        if (mDeviceImage == null) {
            mDeviceImage = imageFactory.getImageByName("device.png"); //$NON-NLS-1$
        }
        if (mEmulatorImage == null) {
            mEmulatorImage = imageFactory.getImageByName("emulator.png"); //$NON-NLS-1$
        }

        if (mMatchImage == null) {
            mMatchImage = mIconFactory.getColorIcon("match", //$NON-NLS-1$
            		SWT.COLOR_DARK_GREEN);
        }

        if (mNoMatchImage == null) {
            mNoMatchImage = mIconFactory.getColorIcon("error", //$NON-NLS-1$
            		SWT.COLOR_DARK_RED);
        }

        if (mWarningImage == null) {
            mWarningImage = mIconFactory.getColorIcon("warning", //$NON-NLS-1$
                    SWT.COLOR_YELLOW);
        }
        ...
        public boolean accept(AvdAgent avdAgent) {
            ...
            if (mDevices != null) {
                for (IDevice d : mDevices) {
                    ...
                    Utilities.Compatibility c =
                    		Utilities.canRun(avdAgent.getAvd(), getAndroidTargetFor(info), mProjectTarget, mMinApiVersion);
                    return (c == Utilities.Compatibility.NO) ? false : true;
                }
            }
