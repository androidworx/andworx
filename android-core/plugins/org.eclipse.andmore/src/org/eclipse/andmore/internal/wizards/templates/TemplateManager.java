...
public class TemplateManager {
    ...
    public static File getTemplateRootFolder() {
    	...
    	if (androidEnv.isValid()) {
            File folder = new File(androidEnv.getAndroidSdkHandler().getLocation(), FD_TOOLS + File.separator + FD_TEMPLATES);
    ...
    public static List<File> getExtraTemplateRootFolders() {
    	...
    	AndroidEnvironment androidEnv =  AndworxFactory.instance().getAndroidEnvironment();
    	if (androidEnv.isValid()) {
            File extras = new File(androidEnv.getAndroidSdkHandler().getLocation(), FD_EXTRAS);
    ...
    TemplateMetadata getTemplate(File templateDir) {
        ...
        try {
            ...
            if (templateFile.isFile()) {
                String xml = Files.asCharSource(templateFile, Charsets.UTF_8).read();
