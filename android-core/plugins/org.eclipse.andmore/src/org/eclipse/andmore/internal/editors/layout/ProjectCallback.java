...
public final class ProjectCallback extends LayoutlibCallback {
    ...
    public com.android.util.Pair<ResourceType, String> resolveResourceId(int id) {
        if (mProjectRes != null) {
        	Pair<ResourceType, String> resource = mProjectRes.resolveResourceId(id);
        	// This is used for backward compatibility with layoutlib-api
            return com.android.util.Pair.of(resource.getFirst(), resource.getSecond());
        }
        return null;
    }
                        ...
                        if (constructorSignature.length < 2 && mLogger != null) {
                            mLogger.warning("wrongconstructor", //$NON-NLS-1$
                                String.format("Custom view %1$s is not using the 2- or 3-argument "
                                    ...
                                    clazz.getSimpleName()), null /*data*/, null);
                        }
                        break;
                    }
    ...
    private ILayoutPullParser getParser(String layoutName, File xml) {
        ...
        if (xml != null && xml.isFile()) {
            ...
            try {
                ...
                String xmlText = Files.asCharSource(xml, Charsets.UTF_8).read();
