...
class SampleSelectionPage extends WizardPage implements SelectionListener, ModifyListener {
    ...
    public void setVisible(boolean visible) {
        ...
        if (visible) {
            if (mValues.getProjectName() != null) {
                try {
                    mIgnore = true;
                    mSampleProjectName.setText(mValues.getProjectName());
                } finally {
                    mIgnore = false;
                }
            }
            ...
            // been populated)
            if (mCurrentSamplesTarget != mValues.getTarget()) {
                mCurrentSamplesTarget = mValues.getTarget();
                updateSamples();
            }
    ...
    private void updateSamples() {
        ...
        if (mValues.getSamples() != null && mValues.getSamples().size() > 0) {
            Object[] samples = mValues.getSamples().toArray();
            mTableViewer.setInput(samples);

            mTable.select(0);
            selectSample(mValues.getSamples().get(0).getSecond());
            extractNamesFromAndroidManifest();
        }
    }

    private void selectSample(File sample) {
        mValues.setChosenSample(sample);
        if (sample != null && !mValues.isProjectNameModifiedByUser()) {
            mValues.setProjectName (sample.getName());
            if (SdkConstants.FD_SAMPLE.equals(mValues.getProjectName()) &&
                    sample.getParentFile() != null) {
                mValues.setProjectName(sample.getParentFile().getName() + '_' + mValues.getProjectName());
            }
            try {
                mIgnore = true;
                mSampleProjectName.setText(mValues.getProjectName());
    ...
    public void modifyText(ModifyEvent e) {
        ...
        if (e.getSource() == mSampleProjectName) {
            mValues.setProjectName(mSampleProjectName.getText().trim());
    ...
    private void updatedProjectName() {
        if (mValues.isUseDefaultLocation()) {
            mValues.setProjectLocation(Platform.getLocation().toFile());
        }
    }
    ...
    private void extractNamesFromAndroidManifest() {
        if (mValues.getChosenSample() == null || !mValues.getChosenSample().isDirectory()) {
            return;
        }

        Path path = new Path(mValues.getChosenSample().getPath());
    ...
    public boolean isPageComplete() {
        ...
        if (mValues.getCosenSample() == null) {
            return false;
        }
    ...
    private void validatePage() {
        ...
        } else if (mValues.getChosenSample ()== null) {
            status = new Status(IStatus.ERROR, AndmoreAndroidConstants.PLUGIN_ID, "Choose a sample");
        } else if (!mValues.getChosenSample().exists()) {
            status = new Status(IStatus.ERROR, AndmoreAndroidConstants.PLUGIN_ID,
                    String.format("Sample does not exist: %1$s", mValues.getChosenSample().getPath()));
        } else {
            status = ProjectNamePage.validateProjectName(mValues.getProjectName());
        }
