...
public class Configuration {
    ...
    public final static QualifiedName NAME_RENDER_STATE =
        new QualifiedName(AndmoreAndroidConstants.ANDMORE_ID, "render");          //$NON-NLS-1$
        ...
        try {
            ...
            if (data != null) {
                ...
                if (values.length == 2) {
                    ...
                    if (adtPrefs.isAutoPickRenderTarget()) {
     ...
     * @return true if the Features is supported
     */
    public boolean supports(int capbability) {
        IAndroidTarget target = getTarget();
        if (target != null) {
            return RenderService.supports(target, capbability);
        }
