...
class SdkSelectionPage extends WizardPage implements ITargetChangeListener {
    ...
    public void createControl(Composite parent) {
        ...
        mSdkTargetSelector.setSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                mValues.setTarget(mSdkTargetSelector.getSelected());
                mValues.setTargetModifiedByUser(true);
                onSdkTargetModified();
                validatePage();
            }
        });
    ...
    public void setVisible(boolean visible) {
        ...
        try {
            mIgnore = true;
            if (mValues.getTarget() != null) {
                mSdkTargetSelector.setSelection(mValues.getTarget());
            }
        } finally {
            mIgnore = false;
        }
    ...
    private void loadSamplesForTarget(IAndroidTarget target) {
        ...
        File previouslyChosenSample = mValues.getChosenSample();

        mValues.getSamples().clear();
        mValues.setChosenSample(null);
        ...
        if (target != null) {
            ...
            findSamplesManifests(root, root, null, null, mValues.getSamples());
                ...
                for (Entry<File, String> entry : extras.entrySet()) {
                    ...
                    if (path.getName().equals(SdkConstants.FD_SAMPLE)) {
                        findSampleManifestInDir(
                                path, path, name, target.getVersion(), mValues.getSamples());
                    }
            ...
            if (mValues.getSamples().isEmpty()) {
                return;
            } else {
                Collections.sort(mValues.getSamples(), new Comparator<Pair<String, File>>() {
                    @Override
            ...
            if (previouslyChosenSample != null) {
                ...
                for (int i = 0, n = mValues.getSamples().size(); i < n; i++) {
                    File file = mValues.getSamples().get(i).getSecond();
                    if (file.getName().equals(previouslyChosenName)) {
                        mValues.setChosenSample(file);
                        break;
                    }
                }
            }
    ...
    private void validatePage() {
        ...
        if (error == null && mValues.getTarget() == null) {
            error = "An SDK Target must be specified.";
        }

        if (error == null && mValues.getMode() == Mode.SAMPLE) {
    ...
    public void onSdkLoaded() {
        ...
        AndroidEnvironment env = AndworxFactory.instance().getAndroidEnvironment();
               IAndroidTarget[] targets = env.getAndroidTargets().toArray(EMPTY_TARGET_ARRAY);
        ...
        if (targets.length == 1) {
            ...
            if (initialTarget != null) {
                mValues.setTarget(initialTarget);
                try {
                    mIgnore = true;
                    mSdkTargetSelector.setSelection(mValues.getTarget());
                } finally {
