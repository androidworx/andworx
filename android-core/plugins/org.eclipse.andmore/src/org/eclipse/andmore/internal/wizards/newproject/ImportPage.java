...
class ImportPage extends WizardPage implements SelectionListener, IStructuredContentProvider,
    ...
    public void createControl(Composite parent) {
        ...
        mCheckboxTableViewer.setLabelProvider(new ProjectCellLabelProvider(mTable, mCheckboxTableViewer));
    ...
    private void refresh() {
        ...
        if (!mProjectPaths.isEmpty())
        	mValues.setTarget(mProjectPaths.get(0).getProjectProfile().getTarget());
    ...
    private void addProjects(File dir, List<ImportedProject> projects, int prefixLength) {
        if (dir.isDirectory()) {
            if (LintUtils.isManifestFolder(dir)) {
                ...
                ImportedProject importedProject = new ImportedSubProject(dir, relative);
                // Get profile now as this can be a long-running operation if libararies need to be created
                importedProject.getProjectProfile();
                ...
                if (mCheckboxTableViewer.getGrayed(project)) {
                    ...
                    if (status != null && !status.isOK()) {
                        ...
                        if (mValues.importProjects.size() > 1) {
                            ...
                            status = new Status(status.getSeverity(), AndmoreAndroidConstants.PLUGIN_ID,
                                    message);
                        }
                        break;
                    } else {
    ...
    public void widgetSelected(SelectionEvent e) {
        ...
        if (source == mBrowseButton) {
            ...
            mValues.setCopyIntoWorkspace(mCopyCheckBox.getSelection());
        }
    ...
    private class ProjectNameEditingSupport extends EditingSupport {
        ...
        protected void setValue(Object element, Object value) {
            ImportedSubProject project = (ImportedSubProject) element;
